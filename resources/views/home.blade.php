...
<main class="w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-12 bg-[#F5F7F4]">
    ...
    <section id="beranda" class="relative w-full overflow-hidden pt-2 pb-4">
        <div class="relative flex items-center justify-center gap-4 min-h-[380px] sm:min-h-[420px]">
            ...
            @if ($topCampaigns->isNotEmpty())
                ...
                <div class="hidden lg:block absolute -left-[280px] xl:-left-[240px] w-[540px] h-[360px] rounded-3xl overflow-hidden shadow-md opacity-70 filter brightness-90 shrink-0 select-none z-10">
                    <img id="heroPreviousImage" src="{{ $campaignImage($leftCampaign) }}" alt="{{ $leftCampaign->judul }}" class="w-full h-full object-cover transition-opacity duration-500">
                    <div class="absolute inset-0 bg-black/40"></div>
                </div>
            @endif

            <div class="relative w-full max-w-[1000px] h-[380px] sm:h-[420px] rounded-3xl overflow-hidden shadow-xl z-20">
                <img id="heroActiveImage" src="{{ $campaignImage($featuredCampaign) }}" alt="{{ $featuredCampaign?->judul ?? 'Kampanye donasi' }}" class="w-full h-full object-cover transition-opacity duration-500">
                <div class="absolute inset-0 bg-gradient-to-r from-black/40 via-transparent to-transparent"></div>

                <!-- Left Overlay Translucent Box -->
                <div id="heroActiveContent" class="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 max-w-[460px] bg-black/55 backdrop-blur-md p-6 sm:p-8 rounded-2xl border border-white/25 text-white shadow-2xl">
                    ...
                    <p class="text-[14px] sm:text-[15px] text-gray-200 mt-3 leading-relaxed">
                        <span id="heroActiveDescription">{{ $featuredCampaign?->deskripsi ?? 'Kampanye donasi akan tampil di halaman ini setelah tersedia.' }}</span>
                    </p>
                    <button class="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#05522d] hover:bg-[#044023] text-white text-sm font-semibold transition-all shadow-md">
            ...
            @if ($topCampaigns->isNotEmpty())
                ...
                <div class="hidden lg:block absolute -right-[280px] xl:-right-[240px] w-[540px] h-[360px] rounded-3xl overflow-hidden shadow-md opacity-70 filter brightness-90 shrink-0 select-none z-10">
                    <img id="heroNextImage" src="{{ $campaignImage($rightCampaign) }}" alt="{{ $rightCampaign->judul }}" class="w-full h-full object-cover transition-opacity duration-500">
    ...
    <section id="bantu-mereka" class="pt-2">
        <div class="flex items-center justify-between mb-5">
            ...
            <div class="flex items-center gap-2">
                <button type="button" onclick="document.getElementById('bantuTrack').scrollBy({left: -360, behavior: 'smooth'})" class="w-9 h-9 rounded-full bg-white border border-gray-200 text-gray-700 hover:text-[#05522d] hover:border-[#05522d] hover:bg-[#d8f0e2]/40 flex items-center justify-center transition-all cursor-pointer shadow-xs active:scale-95">
                    &#10094;
                </button>
                <button type="button" onclick="document.getElementById('bantuTrack').scrollBy({left: 360, behavior: 'smooth'})" class="w-9 h-9 rounded-full bg-[#05522d] text-white hover:bg-[#044023] flex items-center justify-center transition-all cursor-pointer shadow-xs active:scale-95">
                    &#10095;
                </button>
            </div>
        </div>

        <div id="bantuTrack" class="flex gap-6 overflow-x-auto scroll-smooth pb-4 pt-1 px-1 -mx-1 select-none" style="scrollbar-width: none;">
            @forelse ($todayCampaigns as $campaign)
                <div class="w-[300px] sm:w-[350px] md:w-[365px] shrink-0 bg-white rounded-[20px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col justify-between hover:shadow-lg transition-all">
                    <div class="relative h-44 sm:h-48 w-full overflow-hidden bg-gray-100">
                        <img src="{{ $campaignImage($campaign) }}" alt="{{ $campaign->judul }}" class="w-full h-full object-cover">
                    </div>
                    <div class="p-5 flex flex-col flex-1 justify-between">
                        <div>
                            <span class="text-xs text-gray-500 font-medium">{{ $campaign->organization?->nama_lembaga ?? 'Organisasi sosial' }}</span>
                        ...
                        <div class="mt-4 pt-2">
                            <div class="flex items-center gap-1.5 text-sm">
                                ...
                                <span class="font-bold text-[#05522d]">Rp {{ number_format($campaign->total_terkumpul, 0, ',', '.') }}</span>
                            </div>
                            <div class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden mt-2.5">
                                <div class="h-full bg-[#207466] rounded-full" style="width: {{ $campaign->progress }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section id="rekomendasi" class="pt-6">
        ...
        <div class="w-full bg-[#207466] rounded-[24px] overflow-hidden shadow-xl grid grid-cols-1 md:grid-cols-12 items-stretch">
            <div class="md:col-span-7 p-7 sm:p-10 lg:p-12 flex flex-col justify-center text-white space-y-4">
                ...
                <div class="pt-4">
                    <button class="px-7 py-3 rounded-xl bg-[#d87625] hover:bg-[#c2651b] text-white font-bold text-[15px] sm:text-[16px] transition-all shadow-md">
            ...
            <div class="md:col-span-5 relative min-h-[260px] sm:min-h-[320px] bg-black">
                <img src="{{ $campaignImage($featuredCampaign) }}" alt="{{ $featuredCampaign?->judul ?? 'Rekomendasi donasi' }}" class="w-full h-full object-cover">
            </div>
        </div>
    </section>
    ...
    <section id="prioritas" class="pt-6 pb-8">
        <div class="flex items-center justify-between mb-5">
            ...
            <div class="flex items-center gap-2">
                <button type="button" onclick="document.getElementById('prioritasTrack').scrollBy({left: -360, behavior: 'smooth'})" class="w-9 h-9 rounded-full bg-white border border-gray-200 text-gray-700 hover:text-[#05522d] hover:border-[#05522d] hover:bg-[#d8f0e2]/40 flex items-center justify-center transition-all cursor-pointer shadow-xs active:scale-95">
                    &#10094;
                </button>
                <button type="button" onclick="document.getElementById('prioritasTrack').scrollBy({left: 360, behavior: 'smooth'})" class="w-9 h-9 rounded-full bg-[#05522d] text-white hover:bg-[#044023] flex items-center justify-center transition-all cursor-pointer shadow-xs active:scale-95">
                    &#10095;
                </button>
            </div>
        </div>

        <div id="prioritasTrack" class="flex gap-6 overflow-x-auto scroll-smooth pb-4 pt-1 px-1 -mx-1 select-none" style="scrollbar-width: none;">
            @forelse ($priorityCampaigns as $campaign)
                <div class="w-[300px] sm:w-[350px] md:w-[365px] shrink-0 bg-white rounded-[20px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col justify-between hover:shadow-lg transition-all">
                    <div class="relative h-44 sm:h-48 w-full overflow-hidden bg-gray-100">
                        <img src="{{ $campaignImage($campaign) }}" alt="{{ $campaign->judul }}" class="w-full h-full object-cover">
                        <div class="absolute top-3 right-3 bg-[#fceeed] text-[#b02a37] border border-[#f5c6cb] px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm">
            ...
            <div class="w-[300px] sm:w-[350px] md:w-[365px] shrink-0 bg-white rounded-[20px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col justify-between hover:shadow-lg transition-all">
                <div class="relative h-44 sm:h-48 w-full overflow-hidden bg-gray-100">
                    <img src="{{ asset('assets/images/kids_eating_pangan_1789963840194.jpg') }}" class="w-full h-full object-cover">
                    <div class="absolute top-3 right-3 bg-[#fceeed] text-[#b02a37] border border-[#f5c6cb] px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm">
                ...
                <div class="p-5 flex flex-col flex-1 justify-between">
                    <div>
                        ...
                        <h3 class="text-[16px] font-bold text-gray-900 mt-1 leading-snug">Bantuan Kebutuhan Pangan Anak Panti</h3>
                    </div>
                    <div class="mt-4 pt-2">
                        <div class="flex items-center gap-1.5 text-sm">
                            <span class="text-gray-500">Terkumpul</span>
            ...
            <div class="w-[300px] sm:w-[350px] md:w-[365px] shrink-0 bg-white rounded-[20px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col justify-between hover:shadow-lg transition-all">
                <div class="relative h-44 sm:h-48 w-full overflow-hidden bg-gray-100">
                    <img src="{{ asset('assets/images/classroom_chalkboard_1789963866200.jpg') }}" class="w-full h-full object-cover">
                    <div class="absolute top-3 right-3 bg-[#fceeed] text-[#b02a37] border border-[#f5c6cb] px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm">
                ...
                <div class="p-5 flex flex-col flex-1 justify-between">
                    <div>
                        ...
                        <h3 class="text-[16px] font-bold text-gray-900 mt-1 leading-snug">Renovasi Ruang Belajar yang Layak</h3>
                    </div>
                    <div class="mt-4 pt-2">
                        <div class="flex items-center gap-1.5 text-sm">
                            <span class="text-gray-500">Terkumpul</span>
            ...
            <div class="w-[300px] sm:w-[350px] md:w-[365px] shrink-0 bg-white rounded-[20px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col justify-between hover:shadow-lg transition-all">
                <div class="relative h-44 sm:h-48 w-full overflow-hidden bg-gray-100">
                    <img src="{{ asset('assets/images/orphanage_beds_1789963879401.jpg') }}" class="w-full h-full object-cover">
                    <div class="absolute top-3 right-3 bg-[#fceeed] text-[#b02a37] border border-[#f5c6cb] px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm">
                ...
                <div class="p-5 flex flex-col flex-1 justify-between">
                    <div>
                        ...
                        <h3 class="text-[16px] font-bold text-gray-900 mt-1 leading-snug">Bantuan Perlengkapan Tidur Anak Panti</h3>
                    </div>
                    <div class="mt-4 pt-2">
                        <div class="flex items-center gap-1.5 text-sm">
                            <span class="text-gray-500">Terkumpul</span>
            ...
            <div class="w-[300px] sm:w-[350px] md:w-[365px] shrink-0 bg-white rounded-[20px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col justify-between hover:shadow-lg transition-all">
                <div class="relative h-44 sm:h-48 w-full overflow-hidden bg-gray-100">
                    <img src="{{ asset('assets/images/bookshelf_library_1789963921279.jpg') }}" class="w-full h-full object-cover">
                    <div class="absolute top-3 right-3 bg-[#fceeed] text-[#b02a37] border border-[#f5c6cb] px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm">
                ...
                <div class="p-5 flex flex-col flex-1 justify-between">
                    <div>
                        ...
                        <h3 class="text-[16px] font-bold text-gray-900 mt-1 leading-snug">Paket Nutrisi & Susu Balita Panti Asuhan</h3>
                    </div>
                    <div class="mt-4 pt-2">
                        <div class="flex items-center gap-1.5 text-sm">
                            <span class="text-gray-500">Terkumpul</span>
